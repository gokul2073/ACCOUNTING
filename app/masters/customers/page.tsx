  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Manage customer contacts, GSTIN tax identifiers, addresses, and credit limits.
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {loading ? (
          <div className="p-12 text-center text-slate-400 text-xs font-medium">Loading customers...</div>
        ) : (
            ...
            <tbody className="divide-y divide-slate-200">
              {filtered.map((cust) => (
                <tr key={cust.id} className="hover:bg-slate-50 transition-colors">
                  <td className="p-3 font-mono font-bold text-blue-600">{cust.customerCode}</td>
                  <td className="p-3 font-bold text-slate-900">{cust.name}</td>
                  <td className="p-3 font-mono text-slate-600">{cust.gstin || 'URP'}</td>
                  <td className="p-3">{cust.state} ({cust.stateCode})</td>
                  <td className="p-3 text-slate-600">{cust.phone}</td>
                  <td className="p-3 text-right font-mono font-semibold">₹{cust.creditLimit.toLocaleString('en-IN')}</td>
