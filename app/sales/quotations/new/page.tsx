...
export default function NewQuotationPage() {
  ...
  const [salesperson, setSalesperson] = useState('Rajesh Sharma');
  const [reference, setReference] = useState('');
  const [terms, setTerms] = useState('Valid for 30 days. Payment terms as agreed.');
  const [notes, setNotes] = useState('Thank you for your business inquiry.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        router.push('/sales/orders?tab=quotations');
      } else {
    ...
    <form onSubmit={handleSubmit} className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Create Sales Quotation</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Draft formal price quotation for customers with itemized GST breakdown.
            </p>
          </div>
        </div>
        <button
        ...
        >
          ...
          <span>{saving ? 'Saving Quotation...' : 'Save & Issue Quotation'}</span>
        </button>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <label className="block text-xs font-bold text-slate-700 mt-4 mb-1">Valid Until Date *</label>
          <input
            type="date"
            value={validUntil}
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1">Salesperson / Reference</label>
          <input
            type="text"
          ...
          <input
            type="text"
            placeholder="Customer Enquiry / Ref No."
            value={reference}
