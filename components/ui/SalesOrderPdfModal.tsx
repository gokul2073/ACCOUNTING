...
export interface SalesOrderPdfModalProps {
  isOpen: boolean;
  onClose: () => void;
  salesOrder: any;
...
export default function SalesOrderPdfModal({ isOpen, onClose, salesOrder, onDeleteSuccess }: SalesOrderPdfModalProps) {
  ...
  const isInterState = salesOrder.customer?.stateCode && salesOrder.customer?.stateCode !== '33';
  const companyName = salesOrder.company?.name || 'BALAJI CONVEYORS';
  const companyGstin = salesOrder.company?.gstin || '33AANFB9381J1Z0';
  const companyStateCode = salesOrder.company?.stateCode || '33';
  const companyAddress = salesOrder.company?.address || '504/4 Chinnaelasagiri, Balaji nagar, Sipcot, Hosur - 635 126.';
  ...
  const handleSaveAndDownloadPdf = async () => {
    ...
    setSaveStatus(null);
    try {
      const docNo = salesOrder.orderNumber || 'SalesOrder';
  ...
  const handleDeleteSalesOrder = async () => {
    ...
    const confirmDelete = window.confirm(
      `Are you sure you want to delete Sales Order ${salesOrder.orderNumber}? This action cannot be undone.`
    );
  ...
  const orderDateStr = salesOrder.orderDate
    ? new Date(salesOrder.orderDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-')
    : '';

  const deliveryDateStr = salesOrder.deliveryDate
    ? new Date(salesOrder.deliveryDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-')
    : '';
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[95vh] flex flex-col overflow-hidden border border-slate-200">
        ...
        <div className="px-6 py-3 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800 no-print">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm tracking-tight">Sales Order Confirmation Preview</h3>
            ...
            <div className="border-2 border-black flex-1 flex flex-col justify-between box-border">
                ...
                <div className="grid grid-cols-12 border-b-2 border-black">
                  <div className="col-span-7 border-r-2 border-black p-0 flex flex-col justify-between">
                    ...
                    <div className="p-2 text-[11px] leading-snug space-y-1">
                      <p><span className="font-bold">NAME : </span><strong className="font-extrabold text-black uppercase text-xs">{salesOrder.customer?.name}</strong></p>
                      <p className="text-[10.5px] font-medium text-slate-900">{salesOrder.customer?.billingAddress}</p>
                      <p className="text-[10.5px] font-bold text-black mt-1">
                        GST NO : <span className="font-mono">{salesOrder.customer?.gstin || 'URP'}</span>
                      </p>
                      <p className="text-[10.5px] font-bold text-black">
                        State Code : {salesOrder.customer?.stateCode || '33'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="col-span-5 p-0 text-[10.5px] font-semibold">
                    <div className="grid grid-cols-12 border-b border-black">
                      <div className="col-span-4 p-1 border-r border-black font-bold bg-slate-50">ORDER NO:</div>
                      <div className="col-span-3 p-1 border-r border-black font-mono font-extrabold text-center text-blue-900">{salesOrder.orderNumber}</div>
                      <div className="col-span-2 p-1 border-r border-black font-bold text-center bg-slate-50">DATE</div>
                      <div className="col-span-3 p-1 font-mono font-bold text-center">{orderDateStr}</div>
                    </div>
                    <div className="grid grid-cols-12 border-b border-black">
                      <div className="col-span-4 p-1 border-r border-black font-bold bg-slate-50">Delivery</div>
                      <div className="col-span-8 p-1 font-mono">{deliveryDateStr || 'Standard'}</div>
                    </div>
                    <div className="grid grid-cols-12 border-b border-black">
                      <div className="col-span-4 p-1 border-r border-black font-bold bg-slate-50">Status</div>
                      <div className="col-span-8 p-1 font-mono text-blue-800 font-bold">{salesOrder.status}</div>
                    </div>
                    <div className="grid grid-cols-12 border-b border-black">
                      <div className="col-span-4 p-1 border-r border-black font-bold bg-slate-50">Payment Terms</div>
                      <div className="col-span-8 p-1 font-mono">{salesOrder.terms || 'As per Agreement'}</div>
                    </div>
                ...
                <table className="w-full text-xs border-collapse border-b-2 border-black">
                  <thead>
                    <tr className="bg-slate-100 font-extrabold text-[10.5px] text-center border-b-2 border-black">
                      ...
                      <th className="p-1.5 border-r border-black w-24">HSN/ SAC Code</th>
              ...
              <div>
                ...
                <div className="grid grid-cols-12 border-t-2 border-b-2 border-black">
                  <div className="col-span-7 border-r-2 border-black p-0 flex flex-col justify-between">
                    ...
                    <div className="p-2 text-[10.5px]">
                      ...
                      <span className="font-bold italic font-mono text-black">{numberToWordsINR(salesOrder.grandTotal)}</span>
                    </div>
                  </div>
                  ...
                  <div className="col-span-5 p-0 text-[10.5px] font-semibold">
                    <div className="grid grid-cols-12 border-b border-black">
                      ...
                      <div className="col-span-6 p-1 text-right font-mono font-bold">{salesOrder.taxableAmount.toFixed(2)}</div>
                    </div>
                    {!isInterState ? (
                      <>
                        <div className="grid grid-cols-12 border-b border-black">
                          <div className="col-span-6 p-1 border-r border-black font-bold">SGST 9%</div>
                          <div className="col-span-6 p-1 text-right font-mono">{salesOrder.sgstTotal.toFixed(2)}</div>
                        </div>
                        <div className="grid grid-cols-12 border-b border-black">
                          <div className="col-span-6 p-1 border-r border-black font-bold">CGST 9%</div>
                          <div className="col-span-6 p-1 text-right font-mono">{salesOrder.cgstTotal.toFixed(2)}</div>
                        </div>
                        <div className="grid grid-cols-12 border-b border-black">
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-12 border-b border-black">
                          <div className="col-span-6 p-1 border-r border-black font-bold">IGST 18%</div>
                          <div className="col-span-6 p-1 text-right font-mono">{salesOrder.igstTotal.toFixed(2)}</div>
                        </div>
                      </>
                    )}
                    <div className="grid grid-cols-12 bg-slate-100 font-bold border-t border-black">
                      ...
                      <div className="col-span-6 p-1 text-right font-mono font-black text-xs text-black">{salesOrder.grandTotal.toFixed(2)}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-2 text-[9px] text-justify leading-tight border-b-2 border-black text-slate-800">
                  We certify that our registration certificate under the GST Act 2017 is in force on the date on which the supply of goods specified in this Tax Invoice is made by us & the transaction covered by this Tax Invoice has been effected by us and it shall be accounted for in the turnover of supplies while filing of return and the due tax if any payable on the supplies has been paid or shall be paid. E.&.O.E
                </div>

                {/* Footer Signatory Box */}
                <div className="grid grid-cols-2 p-3 text-[10.5px]">
                  <div className="flex flex-col justify-between">
                    <p className="font-bold text-slate-900">Enclosures: Sales Order Confirmation</p>
